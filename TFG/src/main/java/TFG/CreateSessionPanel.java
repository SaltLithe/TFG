	...
	public void initMainFrame(String nombre) {
		...
		SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				...
				AsynchronousServer server = new AsynchronousServer(n, handler, 5, 8080, 8081, null, 100);
				server.setAutomaticIP();
				server.Start();
				DeveloperMainFrame mainframe = new DeveloperMainFrame(null, server, null);

				mainframe.setSessionOwner(n);
				mainframe.setIpIndicator(server.getOwnAddress());
